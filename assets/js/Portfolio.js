class Topic {
  constructor(element){
    ...
    // Check which data-set was selected and get the value
    if (typeof this.element.dataset.theme !== "undefined") { 
      this.set = "theme";
      this.dataTopic = this.element.dataset.theme; 
    } else if (typeof this.element.dataset.location !== "undefined") { 
      this.set = "location";
      this.dataTopic = this.element.dataset.location; 
    } else if (typeof this.element.dataset.planner !== "undefined") { 
      this.set = "planner";
      this.dataTopic = this.element.dataset.planner; 
    }
    ...
    if ( this.dataTopic === 'all' ){
      ...
      this.articles = document.querySelectorAll(`.content-container article[data-${this.set}="${this.dataTopic}"]`);
    }
